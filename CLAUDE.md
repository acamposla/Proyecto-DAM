...
- **CTA Soporte:** Cierre dirigiendo a `customer.garza.es` para soporte adicional (sustituye al "Cierre" genérico anterior).
Esto crea consistencia en la experiencia de usuario y facilita la producción por lotes.

### VCF (Configuración): Jerarquía Esencial vs Avanzada
En los playbooks de vídeo VCF se estableció un criterio de priorización:
...
Se pueden bundlelizar varias configs breves en un solo vídeo si son del mismo nivel de criticidad. Badges visuales "ESENCIAL"/"AVANZADO" en pantalla para claridad.
...
5. ⬜ **Definir estrategia multiidioma para vídeos:** Evaluar opciones de producción localizable (capas separadas, voz AI vs actor). Herramientas candidatas: ElevenLabs, HuggingFace, Whisper. Por decidir.
6. ⬜ **Resolver ambigüedad identificadores:** Implementar lookup a BD para casos como "DALIA" vs "SOLAR" (requiere cliente API funcional).
7. ⬜ **Integración completa dam_ingest.py:** Conectar parser + cliente real + manejo de errores + rate limits.
8. ⬜ **Prueba piloto:** 10 activos reales (5 imágenes + 5 vídeos) contra Sales Layer de staging/producción.
9. ⬜ **Documentación para agencias:** Guía de nomenclatura consolidada + proceso de subida (incorporar playbooks de vídeo).
